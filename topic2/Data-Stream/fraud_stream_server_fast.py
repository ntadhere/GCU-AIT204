#!/usr/bin/env python3
"""
Modified Fraud Stream Server - Optimized for Data Collection
=============================================================
This version has higher daily limits and more consistent transaction generation
to avoid "freezing" during long collection sessions.

Key changes from original:
- Daily limits increased 3x
- Lower frequency multipliers to maintain consistent flow
- Better probability balancing

Usage:
    python3 fraud_stream_server_fast.py --interval 0.1
"""
...
class UserProfile:
    ...
    def setup_profile(self):
        profiles = {
            "high_freq_big_spender": {
                ...
                "daily_limit": 60,  # Increased from 20
            ...
            "low_freq_big_spender": {
                ...
                "peak_hours": [11, 15, 19],
                "item_categories": ["E", "F"],
                "daily_limit": 15,  # Increased from 5
            ...
            "high_freq_low_spender": {
                ...
                "item_categories": ["C", "B"],
                "daily_limit": 45,  # Increased from 15
            ...
            "periodic_big_spender": {
                ...
                "daily_limit": 30,  # Increased from 10
                "fraud_probability": 0.01,
            ...
            "morning_shopper": {
                ...
                "item_categories": ["C", "D"],
                "daily_limit": 24,  # Increased from 8
                "fraud_probability": 0.01
            },
            "night_owl": {
                "frequency": 0.6,
                ...
                "item_categories": ["E", "B"],
                "daily_limit": 21,  # Increased from 7
            ...
            "weekend_warrior": {
                ...
                "daily_limit": 36,  # Increased from 12
                "fraud_probability": 0.02,
                "weekend_multiplier": 2.0  # Reduced from 3.0
            },
            "budget_conscious": {
                "frequency": 0.5,
                "amount_range": (5, 30),
                ...
                "peak_hours": [11, 17, 18],
                "item_categories": ["B", "C"],
                "daily_limit": 18,  # Increased from 6
            ...
            "consistent_regular": {
                ...
                "peak_hours": [12, 18],
                "item_categories": ["C"],
                "daily_limit": 6,  # Increased from 2
                "fraud_probability": 0.005
            },
            "seasonal_shopper": {
                "frequency": 0.3,
                ...
                "daily_limit": 18,  # Increased from 6
                "fraud_probability": 0.02,
                "seasonal_multiplier": 2.0  # Reduced from 2.5
            },
            "micro_transactor": {
                "frequency": 0.9,
                "amount_range": (1, 10),
                ...
                "item_categories": ["B"],
                "daily_limit": 90,  # Increased from 30
                "fraud_probability": 0.008
            },
            "luxury_buyer": {
                "frequency": 0.2,
    ...
    def should_transact(self, current_hour, current_minute):
        """Determine if user should make a transaction (optimized for consistent flow)"""
        ...
        # Check weekday_only restriction
        ...
        # Peak hours get boost, but less extreme than original
        if current_hour in self.params["peak_hours"]:
            base_prob *= 1.3  # Reduced from 1.5
        else:
            base_prob *= 0.5  # Increased from 0.3
        ...
        # Time throttling (less aggressive)
        time_since_last = (datetime.now() - self.last_transaction_time).seconds
        if time_since_last < 30:  # Reduced from 60
            base_prob *= 0.3  # Increased from 0.1
        elif time_since_last < 120:  # Reduced from 300
            base_prob *= 0.7  # Increased from 0.5
        
        return random.random() < (base_prob / 60)
    
    def generate_transaction(self, transaction_id):
        """Generate a normal or fraudulent transaction"""
    ...
    def generate_fraud_transaction(self, transaction_id):
        ...
        if pattern == "sudden_spike":
            ...
            amount = random.choice([100, 200, 250, 300, 500, 1000])
        elif pattern == "duplicate_amount":
            amount = round(random.uniform(*self.params["amount_range"]), 2)
...
class FraudStreamServer:
    ...
    def generate_transaction(self):
        """Generate transaction from active users"""
        ...
        if not active_users:
            # If no active users, force at least one random user
            # This prevents "freezing" during data collection
            user = random.choice(list(self.users.values()))
            if user.daily_count < user.params["daily_limit"] * 0.8:  # Allow up to 80% of limit
    ...
    def broadcast_to_clients(self, message):
        """Send transaction to all connected clients"""
        if message is None:
            return
    ...
    def data_generator_thread(self):
        ...
        print("OPTIMIZED FOR DATA COLLECTION - Higher limits, consistent flow")
        fraud_count = 0
        total_count = 0
        ...
        while self.running:
            ...
            if transaction:
                ...
                if transaction.get("isFraud", False):
                    ...
                    print(f"Processed {total_count} transactions ({fraud_count} fraudulent, {(fraud_count/total_count)*100:.1f}%)")
    ...
    def handle_client_connection(self, client_socket, client_addr):
        ...
        welcome_message = {
            "type": "connection",
            "message": "Connected to OPTIMIZED fraud detection training stream",
            "timestamp": datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Optimized Fraud Detection Training Stream Server')
    parser.add_argument('--host', default='localhost', help='Host to bind to (default: localhost)')
    parser.add_argument('--port', type=int, default=5555, help='Port to bind to (default: 5555)')
    parser.add_argument('--interval', type=float, default=0.1, help='Interval between transactions (default: 0.1 for fast collection)')
    ...
    print("=" * 60)
    print("OPTIMIZED FRAUD DETECTION TRAINING STREAM SERVER")
    print("=" * 60)
    print(f"Host: {args.host}")
    print(f"Port: {args.port}")
    print(f"Interval: {args.interval} seconds (~{1/args.interval:.0f} trans/sec)")
    print("=" * 60)
    print("\n⚡ OPTIMIZATIONS:")
    print("- Daily limits increased 3x")
    print("- More consistent transaction generation")
    print("- Fallback to prevent freezing")
    print("- Recommended for data collection")
    print("=" * 60)
